...
function Services() {
    return (
        <div>
            <NavBar/>
            {/* Title */}
            <div style={{fontFamily: "NonSans", fontSize: 40, margin: 20}}>Services</div>
            {/* Services */}
                ...
                {SERVICES.map((service, index) => {
                    return (
                        <Card onClick={() => {
                            ...
                            <img src={require(`../images/${service.image}`)} style={{width: "100%", height: 275, borderRadius: 20}}/>
                            <div style={{display: "flex", flexDirection: "row", alignItems: "center", paddingTop: 10}}>
                                <div style={{fontFamily: "NonSans", fontSize: 30, margin: 10}}>{service.name}</div>
                                <Button style={{height: 40, backgroundColor: "#4663ac", color: "#e1f1fd", margin: 10, marginLeft: "auto"}}
